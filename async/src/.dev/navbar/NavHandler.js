    ...
    enableDisplayToggle(){
        ...
        displayToggle.addEventListener('click', () => {
            if(this.focus){
                ...
                global.geotab.addin.async.blur();
            } else {
                displayToggle.innerHTML = 'Blur add-in';
                global.geotab.addin.async.focus(global.api, global.state);
            }
    ...
    listenToHash(){
        ...
        setInterval(()=>{
            if(window.location.hash !== this.storedHash){
                this.storedHash = window.location.hash;
                if(this.storedHash !== '#' + 'async'){
                    geotab.addin.async.blur();
                } else {
                    geotab.addin.async.focus(global.api, global.state);
                }
            }
        }, 100);
    }

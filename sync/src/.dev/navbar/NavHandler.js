    ...
    enableDisplayToggle(){
        ...
        displayToggle.addEventListener('click', () => {
            if(this.focus){
                ...
                global.geotab.addin.sync.blur();
            } else {
                displayToggle.innerHTML = 'Blur add-in';
                global.geotab.addin.sync.focus(global.api, global.state);
            }
    ...
    listenToHash(){
        ...
        setInterval(()=>{
            if(window.location.hash !== this.storedHash){
                this.storedHash = window.location.hash;
                if(this.storedHash !== '#' + 'sync'){
                    geotab.addin.sync.blur();
                } else {
                    geotab.addin.sync.focus(global.api, global.state);
                }
            }
        }, 100);
    }
